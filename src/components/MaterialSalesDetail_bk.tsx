...
export default function MaterialSalesDetailProducts({
    productId,
  }: ProductDetailProps) {
    ...
    // JSONファイルのインポート形式に対応
    const materialsData = Array.isArray(materialsDataRaw) 
      ? materialsDataRaw 
      : (materialsDataRaw as any).default || [];

    // materialIdに合致するデータを取得
    // materials.jsonのidとproductIdが一致
    const materialData = (materialsData as MaterialsData).find(
      (m) => m.id === productId
    );
    ...
    if (!materialData) {
      return (
        <div>
          ...
          <p className="text-muted-foreground">この商品のデータが見つかりません</p>
        </div>
      );
    }
    ...
    const product = {
      id: productId,
      salesFloorHistory: materialData.salesFloorHistory || [],
    };
